...
// 版本: 2.3.0 (代号: Chimera Synthesis - Multi-language Edition)
...
.lang-btn { width: 48%; padding: 8px; background: #333; color: var(--text); margin-bottom: 10px; border: 1px solid #444; }
.lang-btn.active { background: var(--primary); color: #000; }

.chat-window { flex: 1; background: #000; border: 1px solid var(--border); border-radius: 8px; padding: 20px; overflow-y: auto; display: flex; flex-direction: column; gap: 15px; min-height: 200px; }
...
.history-item { margin: 10px 0; padding: 10px; background: #1a1a1a; border-radius: 6px; border: 1px solid #333; }
.history-item video { max-height: 120px; }
.history-meta { font-size: 12px; color: #888; margin-top: 5px; }
</style>
</head>
<body>
<div class="sidebar">
...
<button id="lang-zh" class="lang-btn active" onclick="setLanguage('zh')">中文</button>
<button id="lang-en" class="lang-btn" onclick="setLanguage('en')">English</button>
</div>

<div class="box">
<span class="label" data-i18n="apiKey">API 密钥</span>
<div class="code-block" onclick="copy('${apiKey}')">${apiKey}</div>
</div>

<div class="box">
<span class="label" data-i18n="apiEndpoint">API 接口地址 (OpenAI 兼容)</span>
<div class="code-block" onclick="copy('${origin}/v1/chat/completions')">${origin}/v1/chat/completions</div>
<div class="api-ref" data-i18n="supportedClients">支持 LobeChat, NextChat</div>
</div>

<div class="box">
<span class="label" data-i18n="comfyEndpoint">ComfyUI / 绘图接口</span>
...
<div class="box">
<span class="label" data-i18n="modeLabel">风格模式 (Mode)</span>
<select id="mode">
<option value="normal" data-i18n-option="modeNormal">Normal (标准)</option>
<option value="fun" data-i18n-option="modeFun">Fun (趣味)</option>
<option value="spicy" data-i18n-option="modeSpicy">Spicy (火辣)</option>
</select>

<span class="label" style="margin-top:10px" data-i18n="ratioLabel">视频比例 (Aspect Ratio)</span>
<select id="ratio">
<option value="1:1" data-i18n-option="ratio1x1">1:1 (方形)</option>
<option value="3:2" data-i18n-option="ratio3x2">3:2 (横屏)</option>
<option value="2:3" data-i18n-option="ratio2x3">2:3 (竖屏)</option>
</select>

<span class="label" style="margin-top:10px" data-i18n="promptLabel">提示词</span>
<textarea id="prompt" rows="4" data-i18n-placeholder="promptPlaceholder" placeholder="描述视频内容..."></textarea>

<button id="btn-gen" onclick="generate()" data-i18n="generateBtn">生成视频</button>
</div>
</div>

<main class="main">
<div class="chat-window" id="chat">
<div id="welcome-msg" style="color:#666; text-align:center; margin-top:50px;">
<span data-i18n="engineReady">Ximagine 文生视频引擎就绪。</span><br>
<span data-i18n="estimatedTime">预计生成时间: 15-30 秒。</span>
</div>
</div>

<div class="box history-panel">
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
<h3 style="margin:0" data-i18n="historyTitle">历史记录</h3>
<button onclick="clearHistory()" style="width:auto;padding:5px 10px" data-i18n="clearHistory">清除历史</button>
</div>
...
zh: {
apiKey: 'API 密钥',
apiEndpoint: 'API 接口地址 (OpenAI 兼容)',
supportedClients: '支持 LobeChat, NextChat',
comfyEndpoint: 'ComfyUI / 绘图接口',
modeLabel: '风格模式 (Mode)',
modeNormal: 'Normal (标准)',
modeFun: 'Fun (趣味)',
modeSpicy: 'Spicy (火辣)',
...
enterPrompt: '请输入提示词',
copied: '已复制',
engineReady: 'Ximagine 文生视频引擎就绪。',
estimatedTime: '预计生成时间: 15-30 秒。',
historyTitle: '历史记录',
clearHistory: '清除历史',
noHistory: '暂无历史记录'
},
...
supportedClients: 'Supports LobeChat, NextChat',
comfyEndpoint: 'ComfyUI / Image API',
...
if (history.length === 0) {
container.innerHTML = '<div style="color:#666;text-align:center;padding:20px">' + t.noHistory + '</div>';
return;
}
...
'<video src="' + h.videoUrl + '" muted loop style="max-height:120px" onmouseover="this.play()" onmouseout="this.pause()"></video>' +
'<div class="history-meta">' + promptShort + '<br>[' + h.mode + ' | ' + h.ratio + '] ' + dateStr + '</div>' +
'<a href="' + h.videoUrl + '" target="_blank" class="download-link">' + t.downloadVideo + '</a>' +
'</div>';
}).join('');
}
...
if (welcomeMsg) {
welcomeMsg.innerHTML = '<span data-i18n="engineReady">' + i18n[lang].engineReady + '</span><br>' +
'<span data-i18n="estimatedTime">' + i18n[lang].estimatedTime + '</span>';
}
...
btn.disabled = true; btn.innerText = t.submitting;

const welcomeMsg = document.getElementById('welcome-msg');
if (welcomeMsg) welcomeMsg.remove();

appendMsg('user', prompt + '<br><small style="color:#888">[' + mode + ' | ' + ratio + ']</small>');
const aiMsg = appendMsg('ai', '<div>' + t.initializing + '</div><div class="progress-bar" style="display:block"><div class="progress-fill" style="width:0%"></div></div>');
...
statusDiv.innerText = t.generatingProgress + ' ' + Math.floor(currentProgress) + '%';
}, 500);
...
aiMsg.innerHTML += '<video src="' + statusData.videoUrl + '" controls autoplay loop><a href="' + statusData.videoUrl + '" target="_blank" class="download-link">' + t.downloadVideo + '</a>';
btn.disabled = false; btn.innerText = t.generateBtn;
...
clearInterval(pollTimer);
clearInterval(fakeProgressTimer);
statusDiv.innerHTML = '<span style="color:#CF6679">' + t.failed + ': ' + statusData.error + '</span>';
btn.disabled = false; btn.innerText = t.generateBtn;
}
} catch (e) {
...
if (fakeProgressTimer) clearInterval(fakeProgressTimer);
statusDiv.innerHTML = '<span style="color:#CF6679">' + t.requestError + ': ' + e.message + '</span>';
btn.disabled = false; btn.innerText = t.generateBtn;
}
}
